...
export function WarRoomMap({ radius = 5, isPlanar = false }) {
    ...
    useEffect(() => {
        ...
        .catch(err => console.error("Manque countries.geojson dans /public"));
    }, []);

    // Drag souris + zoom molette
    useEffect(() => {
        if (!isPlanar) return;
        ...
        const onMouseMove = (e) => {
            if (!glisse || !groupRef.current) return;
            const facteur = camera.position.z * 0.002;
            groupRef.current.position.x += (e.clientX - dernierX) * facteur;
            groupRef.current.position.y -= (e.clientY - dernierY) * facteur;
            dernierX = e.clientX;
            dernierY = e.clientY;
        };
        const onMouseUp = () => {
        ...
        const onWheel = (e) => {
            camera.position.z = Math.max(4, Math.min(50, camera.position.z + e.deltaY * 0.02));
        };
